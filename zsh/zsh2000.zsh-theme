# My old theme, ill keep it because its modified and maybe ill want to use it in a future
CURRENT_BG='NONE'
...
prompt_status() {
  ...
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="%{%F{#000}%K{$BLUE}%}⚙"

  [[ -n "$symbols" ]] && prompt_segment $MAGENTA cyan "$symbols"
}

prompt_time() {
  prompt_segment_right white $MAGENTA '%D{%H:%M:%S} '
}
...
ZSH_THEME_GIT_TIME_SINCE_COMMIT_LONG="%{$fg[magenta]%}"
...
function git_time_since_commit() {
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if [[ $(git log 2>&1 > /dev/null | grep -c "^fatal: bad cyan revision") == 0 ]]; then
            # Get the last commit.
